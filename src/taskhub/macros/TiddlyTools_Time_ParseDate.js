/*\
title: $:/plugins/mabuqian/TaskHub/TiddlyTools/Time/ParseDate.js
...
(function () {
  ...
  exports.parsedate = function (source, operator, options) {
    ...
    source(function (tiddler, title) {
      if (title.match(/^-?\d+$/))
        dt =
          operator.suffix == "unixtime" || operator.suffix == "number"
            ? new Date(Number(title))
            : $tw.utils.parseDate(title);
      else
        dt = new Date(
          title.replace(/(\d+)(st|nd|rd|th)/g, "$1").replace(/,/g, "")
        );
